...
\end{lstlisting}
use the following command in terminal to run the code
find the code required below
\begin{lstlisting}
https://github.com/mohilmukundareddy/EE3900/blob/main/a1/codes/xnyn.c
https://github.com/mohilmukundareddy/EE3900/blob/main/a1/codes/3-2.py
\end{lstlisting}
\begin{lstlisting}
gcc xnyn.c
python3 3-2.py
...
\item Obtain $X(z)$ for $x(n)$ defined in problem 4.1?
 $$ X(z)={\mathcal {Z}}\{x(n)\}=\sum _{n=-\infty }^{\infty }x(n)z^{-n} $$
 \begin{align*}
 X(z) =...&0+x(0)z^{0}+x(1)z^{-1}+x(2)z^{-2}+x(3)z^{-3} \\
           &+x(4)z^{-4}+x(5)z^{-5}+0...     \\
...
\begin{lstlisting}
https://github.com/mohilmukundareddy/EE3900/blob/main/a1/codes/dtft.py
\end{lstlisting}
run the command
\begin{lstlisting}
python3 dtft.py
...
\item Express $h(n)$ in terms of $H\brak{e^{\j\omega}}$ 
 ...
 \solution 
 \begin{align}
  &\int_{-\pi}^{\pi} H(e^{\j\omega}) e^{\j\omega n} \der{d\omega} \\
  &= \int_{-\pi}^{\pi} \sum_{k=-\infty}^\infty h(k)  e^{-\j\omega k} e^{\j\omega n} \der{d\omega} \\
  &= \sum_{k=-\infty}^\infty h(k) \int_{-\pi}^{\pi} e^{\j\omega(n-k)} \der{d\omega}
 \end{align}
 
 Now,
 \begin{align}
   \int_{-\pi}^{\pi} e^{\j\omega(n-k)} \der{d\omega} 
   &= \begin{cases}
    \int_{-\pi}^\pi \der{d\omega} & n-k = 0 \\
...
python3 ynconv_toeplitz.py
    ...
    Using Toeplitz matrix of $h\brak{n}$ we can simplify it as,
     ...
     Now using $\eqref{eq:ynconv_toep}$,
      ...
      \begin{figure}
        \centering
        \includegraphics[scale = 0.8][width = \columnwidth]{figs/toepliz.png}
